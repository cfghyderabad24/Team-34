...
const Screening3 = () => {
    ...
    const handleAccept = (student) => {
        sendEmail(student.name, student.email, 'St. Bhatevara Foundation', 'accepted', student.password);
    };

    const handleReject = (student) => {
        sendEmail(student.name, student.email, 'St. Bhatevara Foundation', 'rejected');
    };
                ...
                <tbody>
                    {students.map((student, idx) => (
                        <tr key={idx}>
                            ...
                            <td>{student.name}</td>
                            <td>
                                <button className="btn btn-success mr-2" onClick={() => handleAccept(student)}>Accept</button> &nbsp;
                                <button className="btn btn-danger" onClick={() => handleReject(student)}>Reject</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
