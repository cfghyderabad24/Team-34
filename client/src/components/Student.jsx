    ...
    return (
        <div className="container mt-5 p-4 bg-light rounded shadow">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <h3>Hey {stdId},</h3>
                <div>
                    <button type="button" className="btn btn-primary mr-2" onClick={() => setShowEditModal(true)}>Edit Details</button> &nbsp;
                    <button type="button" className="btn btn-warning" onClick={() => setShowPasswordModal(true)}>Change Password</button> &nbsp;
                    <button type="button" className="btn btn-secondary" onClick={handleAdminLogin}>Login as Admin</button>
                </div>
            </div>
            ...
            <div className="scholarship-details">
                ...
                <div className="mb-3">
                    ...
                    <h3 className="mt-4">Renewal Deadline: <span style={{ color: "red" }}>{renewal_deadline}</span></h3>
                        ...
                        <tbody>
                            {
                                vcourses.map((item, idx) => (
                                    <tr key={idx}>
                                        <td>{item.course}</td>
                                        <td><strong>{item.attendance}</strong></td>
                                    </tr>
                                ))
                            }
                        </tbody>
                ...
                <Modal.Body>
                    <form onSubmit={handleSubmit(handleEditDetails)}>
                        <div className="form-group">
                            <label>Course</label>
                            <input type="text" className="form-control" defaultValue={course} {...register("course")} />
                        </div>
                        <div className="form-group">
                            <label>Year</label>
                            <input type="number" className="form-control" defaultValue={year} {...register("year")} />
                        </div>
                        <div className="form-group">
                            <label>Scholarship Eligible Amount</label>
                            <input type="number" className="form-control" defaultValue={s_elegible} {...register("s_elegible")} />
                        </div>
                        <div className="form-group">
                            <label>Scholarship Covered</label>
                            <input type="number" className="form-control" defaultValue={s_covered} {...register("s_covered")} />
                        </div>
                ...
                <Modal.Body>
                    <form onSubmit={handleSubmitPassword(handleChangePassword)}>
                        <div className="form-group">
                            ...
                            <input type="password" className="form-control" {...registerPassword("newPassword")} />
                        </div>
                        <button type="submit" className="btn btn-warning mt-3">Change Password</button>
                    </form>
                </Modal.Body>
            </Modal>
        </div>
